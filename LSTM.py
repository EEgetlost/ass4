...
class LSTMModel(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_templates, num_tags):
        ...
        self.lstm = nn.LSTM(
            ...
            hidden_size=embedding_dim // 2,  # 因为是双向的，所以隐藏层大小减半
            num_layers=1,
            batch_first=True,
            bidirectional=True
        )
        ...
        self.template_classifier = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(embedding_dim, num_templates)
        )
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        # 模板分类输出（使用最后一个隐藏状态）
        # 因为是双向LSTM，需要连接前向和后向的最后一个隐藏状态
        last_hidden = torch.cat((hidden[-2], hidden[-1]), dim=-1)
        template_logits = self.template_classifier(last_hidden)
...
def train_model(model, train_loader, dev_loader, num_epochs, learning_rate):
    ...
    for epoch in range(num_epochs):
        ...
        accuracy = 100 * template_correct / template_total
        current_lr = scheduler.get_last_lr()[0]
        print(f"Epoch {epoch+1}/{num_epochs}, Loss: {total_loss/len(train_loader):.4f}, Accuracy: {accuracy:.2f}%, LR: {current_lr:.2e}")
...
# 训练模型
print("\n>>> 训练模型 ...")
train_model(model, train_loader, dev_loader, num_epochs=15, learning_rate=5e-4)  # 增加学习率
