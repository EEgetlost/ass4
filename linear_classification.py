# -*- coding: utf-8 -*-
"""linear_classification.py - 使用线性模型进行SQL模板分类和槽位标注"""
...
class LinearClassificationModel(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_labels):
        super().__init__()
        self.embedding = LinearEmbedding(vocab_size, embedding_dim)
...
class LinearTokenClassificationModel(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_labels):
        super().__init__()
        self.embedding = LinearEmbedding(vocab_size, embedding_dim)
        self.classifier = nn.Linear(embedding_dim, num_labels)
...
cls_model = LinearClassificationModel(VOCAB_SIZE, EMBEDDING_DIM, NUM_TEMPLATES).to(DEVICE)
tok_model = LinearTokenClassificationModel(VOCAB_SIZE, EMBEDDING_DIM, len(label2id)).to(DEVICE)
