# -*- coding: utf-8 -*-
"""lstm_classification.py - 使用LSTM模型进行SQL模板分类和槽位标注"""
...
class LSTMModel(nn.Module):
    ...
    def forward(self, x, attention_mask=None):
        ...
        if attention_mask is not None:
            ...
            return (lstm_out * mask).sum(dim=1) / mask.sum(dim=1)
        else:
            return lstm_out.mean(dim=1)

class LSTMClassificationModel(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, num_labels, num_layers=2, dropout=0.2):
        super().__init__()
        self.lstm = LSTMModel(vocab_size, embedding_dim, hidden_dim, num_layers, dropout)
        self.classifier = nn.Linear(hidden_dim * 2, num_labels)  # *2是因为双向LSTM
...
class LSTMTokenClassificationModel(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, num_labels, num_layers=2, dropout=0.2):
        super().__init__()
        self.embedding = nn.Embedding(vocab_size, embedding_dim)
        self.lstm = nn.LSTM(
...
cls_model = LSTMClassificationModel(VOCAB_SIZE, EMBEDDING_DIM, HIDDEN_DIM, NUM_TEMPLATES).to(DEVICE)
tok_model = LSTMTokenClassificationModel(VOCAB_SIZE, EMBEDDING_DIM, HIDDEN_DIM, len(label2id)).to(DEVICE)
...
# 训练模型
print("\n>>> 训练模板分类器 ...")
train_model(cls_model, train_loader_cls, dev_loader_cls, num_epochs=5, learning_rate=1e-4, is_token_classification=False)

print("\n>>> 训练槽位标注器 ...")
train_model(tok_model, train_loader_tok, dev_loader_tok, num_epochs=5, learning_rate=1e-4, is_token_classification=True)
